 ...
 if(\Auth::user()->role_id=='admin'){
   ...
   $html ='';
    ...
    $html .= '<table cellpadding="0" cellspacing="0" style="line-height: 1;width:100%;">
      ...
      if(!empty($petData->st_passport_size)){
        ...
        <td colspan="3" style="padding: 0px 20px;line-height: 1;text-align:center;">
          <img width="450" style="padding: 0px 20px;" class="myalogo" src="'.$petData->st_passport_size.'" alt="">
        </td>
        </tr><br>';
    ...
    if(!empty($petData->owner_photo)){
      $html.= '<tr>
        <td colspan="3" style="padding: 0px 20px;line-height: 1;text-align:center;">
         <img width="450" style="padding: 0px 20px;" class="myalogo" src="'.$petData->owner_photo.'" alt="">
        </td>
      </tr>';
    ...
    if(!empty($petData->owner_proof_photo)){
      ...
      $html.= '<tr>
        <td colspan="3" style="padding: 0px 20px;line-height: 1;text-align:center;">
         <img width="800" style="padding: 0px 20px;" class="myalogo" src="'.$petData->owner_proof_photo1.'" alt="">
        </td>
      </tr>';
...
public function savePetRegistration(Request $request){
  ...
  // $petData = \DB::table('pet_registration')->where('user_id', \Auth::user()->id)->first();
    ...
    // if(empty($petData)) {
      ...
      if($request->hasFile('owner_proof_photo1')) {
        $img_ext2 = $request->file('owner_proof_photo1')->getClientOriginalExtension();
        $owner_proof_photo_image = 'service_image-' . time() . '.' . $img_ext2;
        $owner_proof_photo = $base_url.'/storage/app/public/pet_document/'.$owner_proof_photo_image;
        $path = $request->file('owner_proof_photo1')->move(storage_path('app/public/pet_document'), $owner_proof_photo_image);
      $insertArray['owner_proof_photo1'] = $owner_proof_photo;
       $insertArray['step3'] = '1';
      ...
      $insert_id = \DB::table("pet_registration")->insertGetId($insertArray);
    ...
    // $petRow =\DB::table("pet_registration")->where('id',$insert_id)->first();
    // $userRow =\DB::table("users")->where('id',$petRow->user_id)->first();

    // try {
    //     $response = $api->createPaymentRequest(array(
    //         "purpose" => "Pet Registration",
    //         "amount" => '10',
    //         "buyer_name" => $userRow->name,
    //         "send_email" => true,
    //         "email" => $userRow->email,
    //         "phone" => $userRow->mobile,
    //         "redirect_url" => route('payment_succes')
    //     ));

    //     $updateArray['payment_request_id'] = $response['id'];

    //     $update = \DB::table('pet_registration')->where('id', $insert_id)->update($updateArray);
...
public function proceed_Payment($id){
  ...
  $petRow =\DB::table("pet_registration")->where('id',$id)->first();
  $userRow =\DB::table("users")->where('id',$petRow->user_id)->first();
  ...
  try {
      ...
      $update = \DB::table('pet_registration')->where('id', $id)->update($updateArray);
...
// success response method.
 public function success(Request $request){
  ...
  // dd($request->all());
    ...
    {
        $updateArray['payment_responce'] = $response_json;

        $update = \DB::table('pet_registration')->where('payment_request_id', $request['payment_request_id'])->update($updateArray);
        return redirect()->route('petForm')->with('error','Payment failed!!');
     }elseif($payment_status=='Credit')
     {
        
        $updateArray['payment_status'] = 'Completed';
        $updateArray['payment_responce'] = $response_json;

        $update = \DB::table('pet_registration')->where('payment_request_id', $request['payment_request_id'])->update($updateArray);
        return redirect()->route('petForm')->with('message','Payment successfully done.');

     }else{
        $updateArray['payment_responce'] = $response_json;

        $update = \DB::table('pet_registration')->where('payment_request_id', $request['payment_request_id'])->update($updateArray);
        return redirect()->route('petForm')->with('error','Opps ! Something went wrong.');
